...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    // }
    const { password, profession, description, salary, ...body } =
      createUserDto;
  ...
  async updateAaccessToken(id: number, accessToken: string) {
    const us = await this.userRepository.findOneBy({ id });
    console.log(id, accessToken);
    
    if (us) {
      this.userRepository.update({ id }, { accessToken });
      return true;
    } else {
      throw new NotFoundException('Oops! user not found');
    }
